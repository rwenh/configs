...
;;;; ── ide-simple: QoL editing commands ───────────────────────────────────────

(with-eval-after-load 'editing-core
  (when (require 'ide-simple
                 (expand-file-name "lib/ide-simple.el" user-emacs-directory) t)
    (global-set-key (kbd "C-c z")   #'ide-simple-monocle)
    (global-set-key (kbd "C-c M-d") #'ide-simple-duplicate-line-or-region)
    (global-set-key (kbd "C-c M-o") #'ide-simple-open-line-below)
    (global-set-key (kbd "C-c M-O") #'ide-simple-open-line-above)
    (global-set-key (kbd "C-c M-.") #'ide-simple-insert-date)
    (global-set-key (kbd "C-c M-r") #'ide-simple-rename-file-and-buffer)
    (global-set-key (kbd "C-c M-c") #'ide-simple-copy-buffer-file-name)
    (global-set-key (kbd "C-c M-s") #'ide-simple-scratch-buffer-for-mode)))

;;;; ── ide-register: quick-file registers from config.yml ────────────────────

(add-hook 'after-init-hook
          (lambda ()
            (when (require 'ide-register
                           (expand-file-name "lib/ide-register.el"
                                             user-emacs-directory) t)
              ;; Populate registers declared in config.yml's `registers' section
              (ide-register-store-quick-files)
              ;; Expose the jump command globally
              (global-set-key (kbd "C-c q j") #'ide-register-quick-jump)
              (global-set-key (kbd "C-c q l") #'ide-register-list-populated)
              (global-set-key (kbd "C-c q s")
                              #'ide-register-save-window-configuration)))
          90)

;;;; ── ide-search: occur and project-grep helpers ─────────────────────────────

(with-eval-after-load 'editing-core
  (when (require 'ide-search
                 (expand-file-name "lib/ide-search.el" user-emacs-directory) t)
    (global-set-key (kbd "C-c s t") #'ide-search-occur-todo-keywords)
    (global-set-key (kbd "C-c s u") #'ide-search-occur-urls)
    (global-set-key (kbd "C-c s l") #'ide-search-occur-long-lines)
    (global-set-key (kbd "C-c s s") #'ide-search-project-grep-symbol)))
...
LIB LAYER  (ide-simple / ide-search / ide-register):
  C-c z     monocle — zoom current window (repeat to restore)
  C-c M-d   duplicate line or region
  C-c M-o   open new line below
  C-c M-O   open new line above
  C-c M-.   insert date (C-u = +time, C-u C-u = Org ts)
  C-c M-r   rename file and buffer
  C-c M-c   copy file path to kill ring (prefix = project-relative)
  C-c M-s   scratch buffer for current mode
  C-c s t   occur: TODO / FIXME / HACK keywords
  C-c s u   occur: URLs
  C-c s l   occur: long lines
  C-c s s   project-wide symbol grep
  C-c q j   quick-jump to registered file (from config.yml registers:)
  C-c q l   list all populated registers
  C-c q s   save current window config to a register

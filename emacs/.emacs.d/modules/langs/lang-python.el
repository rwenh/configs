...
(use-package python
  ...
  ;; NOTE: C-c C-r (send-region) and C-c C-b (send-buffer) are intentionally
  ;; NOT bound here.  FIX #27: C-c C-r clashes with the REST map mnemonic
  ;; (different keymaps, no crash, but confusing).  Use C-c x s (send-region
  ;; via hub) and C-c x b (send-buffer via hub) instead — those are the
  ;; canonical IDE-wide bindings set by keybindings.el.

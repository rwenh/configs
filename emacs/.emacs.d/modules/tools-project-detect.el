...
(defvar emacs-ide-detect--lang-module-overrides
  '(("javascript" . "lang-web")
    ...
    ("csharp"     . "lang-csharp")
    ("dart"       . "lang-dart"))
  "Alist of (LANG-KEY . MODULE-NAME) for non-standard module naming.")
...
(defvar emacs-ide-detect--markers
  '(;; Python
    ...
    ;; Elixir (NEW — was missing from markers)
    ("mix.exs"           . ("elixir"     "lang-functional" nil))
    ;; Scala
    ...
    ;; Ruby (NEW)
    ("Gemfile"           . ("ruby"       "lang-ruby"       nil))
    ("Gemfile.lock"      . ("ruby"       "lang-ruby"       nil))
    ("Rakefile"          . ("ruby"       "lang-ruby"       nil))
    (".ruby-version"     . ("ruby"       "lang-ruby"       nil))
    (".rubocop.yml"      . ("ruby"       "lang-ruby"       nil))
    ;; PHP (NEW)
    ("composer.json"     . ("php"        "lang-php"        nil))
    ("composer.lock"     . ("php"        "lang-php"        nil))
    ("artisan"           . ("php"        "lang-php"        nil))   ; Laravel
    ("wp-config.php"     . ("php"        "lang-php"        nil))   ; WordPress
    ;; C# / .NET (NEW)
    ("global.json"       . ("csharp"     "lang-csharp"     nil))
    ;; Note: .csproj and .sln are detected separately via glob in scan-root
    ;; Dart / Flutter (NEW)
    ("pubspec.yaml"      . ("dart"       "lang-dart"       nil))
    ("pubspec.lock"      . ("dart"       "lang-dart"       nil))
    ;; Prose / config
...
(defvar emacs-ide-detect--ext-map
  '(("py"    . "python") ("pyw"  . "python")
    ...
    ("md"    . "prose") ("org" . "prose")
    ("yml"   . "prose") ("yaml" . "prose")
    ("toml"  . "prose") ("json" . "prose")
    ("html"  . "html") ("htm" . "html")
...
(defun emacs-ide-detect--scan-root (root)
  "Scan ROOT for project marker files. Return deduplicated list of lang keys."
  (when (and root (file-directory-p root))
    (let (found)
      ;; Standard marker files
      ...
      (when (or (directory-files root nil "\\.csproj$")
                (directory-files root nil "\\.sln$"))
        (push "csharp" found))
      (delete-dups found))))
...
(defun emacs-ide-detect-show-status ()
  ...
  (with-output-to-temp-buffer "*Project Detect Status*"
    (princ "=== PROJECT DETECT STATUS (v3.4.0) ===\n\n")
    (let ((root (emacs-ide-detect--project-root)))

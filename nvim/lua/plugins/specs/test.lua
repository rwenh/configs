...
return {
  {
    ...
    dependencies = {
      ...
      "rcasia/neotest-java",
      -- Kotlin: tested via neotest-java (JUnit on JVM). No separate adapter needed.
    },
    keys = {
      ...
      -- Coverage co-located with test keys
    ...
    opts = function()
      ...
      local function load_adapter(name, loader)
        ...
        if not in_rtp then return end
        local ok, adapter = pcall(loader)
        if ok and adapter then table.insert(adapters, adapter)
        else vim.schedule(function() vim.notify("[neotest] adapter failed: " .. name, vim.log.levels.WARN) end) end
      end
      ...
      return { adapters = adapters, status = { virtual_text = true }, output = { open_on_run = true }, quickfix = { open = false } }
    end,
  },
